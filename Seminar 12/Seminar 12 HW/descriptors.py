...
class CheckScores:
    def __init__(self, *, min_sub_score=None, max_sub_score=None, min_test_score=None, max_test_score=None):
    ...
    def validate(self, value):
        ...
        if self.param_name == "_subject_score" and (self.min_sub_score > value or value > self.max_sub_score):
            raise ValueError(f"Некорректная оценка предмета: {value}")
        elif self.param_name == "_test_score" and (self.min_test_score > value or value > self.max_test_score):
